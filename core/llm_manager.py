...
class LLMManager:
    ...
    def _candidate_model_paths(self, model_key: str) -> List[Path]:
        ...
        if primary:
            ...
            # For Mistral: support both dash and dot variants
            if "Mistral" in primary or "mistral" in primary.lower():
    ...
    def choose_model_for_prompt(
        ...
        # Router-gestuetzte Auswahl unter allen verfuegbaren/geladenen Modellen
        try:
    ...
    def load_model(self, model_key: str = "mistral") -> bool:
        with self.model_lock:
            ...
            if not self._is_supported_model_file(model_path):
                self.logger.error(
                    "Modellformat %s wird aktuell nicht unterstuetzt. Bitte konvertieren Sie die Datei in das GGUF-Format und legen Sie sie erneut in %s ab.",
                    model_path.suffix,
                    self.models_dir,
                )
                return False
    ...
    def auto_install_models(self) -> None:
        try:
            ...
            if configured_startup:
                ...
                target_models = [key for key in ("mistral", "deepseek", "qwen", "llama") if key in self.available_models]
