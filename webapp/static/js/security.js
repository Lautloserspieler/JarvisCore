window.addEventListener('DOMContentLoaded', () => {
  ...
  async function fetchSecurityStatus() {
    if (isFileProtocol) {
      updateStatus('Offline-Ansicht: Bitte starte JarvisCore und oeffne diese Seite ueber http://127.0.0.1:5050/static/security.html.', 'warning');
      return;
    }
    if (statusEl && statusEl.dataset.state !== 'warning' && statusEl.dataset.state !== 'success') {
      updateStatus('Pruefe Sicherheitsstatus ...', 'info');
    }
    try {
  ...
  function handleSecurityPayload(security) {
    ...
    if (needsSetup && pending) {
      ...
      updateStatus('Es ist noch keine Authenticator-App hinterlegt. Bitte schliesse die Einrichtung ab.', 'warning');
    } else if (needsSetup) {
  ...
  function showAuthenticatorSetup(pending) {
    ...
    if (qrEl) {
      ...
      if (window.QRCode && pending.provisioning_uri) {
        ...
        if (pending.provisioning_uri && !isFileProtocol) {
          ...
          img.src = `/api/security/authenticator/qr?ts=${Date.now().toString()}`;
  ...
  function showTokenWarning() {
    ...
    updateStatus('Bitte gib zuerst den Web-Token ein, bevor du den Authenticator verknuepfst.', 'warning');
  }
  ...
  async function confirmSetup() {
    ...
    if (isFileProtocol) {
      showError('Code-Bestaetigung ist im Offline-Modus nicht moeglich. Oeffne die Weboberflaeche ueber http://127.0.0.1:5050/static/security.html.');
      updateStatus('Zur Bestaetigung bitte die Weboberflaeche ueber den lokalen Server oeffnen.', 'warning');
      return;
    }
    hideElement(errorEl);
    ...
    try {
      ...
      console.error('Bestaetigung fehlgeschlagen', err);
      showError('Der Code konnte nicht ueberprueft werden.');
      updateStatus('Der Code konnte nicht ueberprueft werden.', 'danger');
    } finally {
  ...
  function defaultHeaders() {
    const token =
      window.sessionStorage.getItem('jarvisWebToken') ||
      window.localStorage.getItem('jarvisWebToken') ||
      (document.getElementById('auth-token') ? document.getElementById('auth-token').value.trim() : '');
    return token
      ? {
          'X-Auth-Token': token,
        }
  ...
  if (openBtn) {
    openBtn.addEventListener('click', async () => {
      ...
      if (isFileProtocol) {
        updateStatus('Offline-Modus: Bitte oeffne die Weboberflaeche ueber den laufenden JarvisCore-Server.', 'warning');
        return;
      }
  ...
  function appendFallbackMessage(container, uri, secret) {
    ...
    if (uri) {
      ...
      link.target = '_blank';
      link.rel = 'noreferrer';
      link.textContent = 'QR-Code extern oeffnen';
      fallback.appendChild(link);
    }

    if (secret) {
      const secretWrapper = document.createElement('small');
      secretWrapper.textContent = 'Geheimschluessel: ';
