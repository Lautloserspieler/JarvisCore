...
class WebInterfaceServer:
    ...
    def __init__(self, jarvis: Any, config: Dict[str, Any], *, logger: Any) -> None:
        ...
        self.token = raw_token.strip() if isinstance(raw_token, str) and raw_token.strip() else None
        guest_flag = bool(self.config.get("allow_guest_commands", False))
        if guest_flag:
            self.logger.warning("allow_guest_commands wird ignoriert; ein Zugriffstoken ist Pflicht.")
        self.allow_guest_commands = False
    ...
    async def _handle_status(self, request: web.Request) -> web.Response:
        ...
        if requires_token and not self._verify_token(request):
            return self._unauthorized()
    ...
    async def _handle_security_status(self, request: web.Request) -> web.Response:
        ...
        if not needs_setup and not self._verify_token(request):
            return self._unauthorized()
    ...
    def _authorize_read(self, request: web.Request) -> bool:
        if not self._check_network_guards(request):
            return False
        return self._verify_token(request)
