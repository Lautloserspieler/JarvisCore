...
function applyTheme(mode) {
    ...
    if (dom.themeToggle) {
        ...
        dom.themeToggle.title = theme === "light" ? "Auf Dark umschalten" : "Auf Light umschalten";
    }
    try {
        localStorage.setItem(THEME_STORAGE_KEY, theme);
    } catch (error) {
        console.debug("Theme konnte nicht gespeichert werden", error);
    }
}
...
function primeTokenFromStorage() {
    if (!dom.tokenInput) return;
    const saved = sessionStorage.getItem("jarvisWebToken");
    const cleaned = saved ? saved.trim() : "";
    if (cleaned) {
        if (dom.tokenInput.value.trim() !== cleaned) {
            dom.tokenInput.value = cleaned;
        }
    } else if (saved !== null && !cleaned) {
        // leeres Token aufraeumen
        sessionStorage.removeItem("jarvisWebToken");
    }
}
...
function persistToken() {
    ...
    if (value) {
        sessionStorage.setItem("jarvisWebToken", value);
    } else {
        sessionStorage.removeItem("jarvisWebToken");
    }
}
...
function updateRuntimeStatus(runtime) {
    if (!runtime) {
        setStatusBadge("Unbekannt", false, "danger");
    ...
    const tone = listening || ready ? "success" : state === "processing" ? "warn" : "danger";
    setStatusBadge(label, active, tone);
    if (runtime.speech_status) {
        updateSpeechControls(runtime.speech_status);
    }
}

function setStatusBadge(text, active, tone = "info") {
    if (!dom.statusBadge) return;
    dom.statusBadge.textContent = text;
    dom.statusBadge.classList.toggle("active", Boolean(active));
    dom.statusBadge.classList.remove("success", "warn", "danger");
    if (tone && tone !== "info") {
        dom.statusBadge.classList.add(tone);
    }
}
...
function appendMessage(entry) {
    ...
    if (entry.role === "assistant") {
        ...
        goodBtn.setAttribute("aria-label", "Antwort war hilfreich");
        goodBtn.addEventListener("click", () => submitFeedback(1, entry.text));
        const badBtn = document.createElement("button");
        badBtn.type = "button";
        badBtn.textContent = "Schlecht";
        badBtn.setAttribute("aria-label", "Antwort war nicht hilfreich");
...
function buildWebsocketCandidates() {
    ...
    const candidates = [];
    const envUrl = window.JARVIS_GATEWAYD_WS || window.JARVIS_GATEWAYD_URL;
    if (envUrl && typeof envUrl === "string") {
        candidates.push(`${envUrl.replace(/\\/$/, "")}${envUrl.includes("?") ? "&" : "?"}${query.replace(/^\\?/, "")}`.replace(/\?$/, ""));
    }
    const gwPort = "7081";
    const host = window.location.hostname;
    const currentPort = window.location.port;
    if (!currentPort || currentPort !== gwPort) {
        candidates.push(`${protocol}://${host}:${gwPort}/ws${query}`);
    }
    candidates.push(`${protocol}://${window.location.host}/events${query}`);
...
function connectWebsocket() {
    ...
    const urls = buildWebsocketCandidates();
    let attempt = 0;
    ...
    const tryConnect = () => {
        ...
        try {
            ...
            console.debug("WS Verbindung fehlgeschlagen", err);
            tryConnect();
        }
    };
...
function renderCrawlerDocuments(list) {
    ...
    if (!list || !list.length) {
        const li = document.createElement("li");
        li.className = "muted";
        li.textContent = "Keine Dokumente verfuegbar";
...
async function handleCrawlerJobSubmit(event) {
    ...
    try {
        ...
        showToast(`Crawler-Job fuer "${topic}" gestartet.`, "success");
    } catch (error) {
...
async function loadSettings() {
    ...
    if (settings.web_interface) {
        ...
        const configuredHost = settings.web_interface.host;
        dom.webForm.elements.host.value = configuredHost && configuredHost.trim() ? configuredHost : "127.0.0.1";
        dom.webForm.elements.token.value = settings.web_interface.token ?? "";
        // Token aus Settings immer vorbefuellen und speichern
        if (dom.tokenInput) {
            const configured = (settings.web_interface.token || "").trim();
            if (configured && dom.tokenInput.value.trim() !== configured) {
                dom.tokenInput.value = configured;
                persistToken();
            }
        }
    }
    if (dom.uiForm) {
        const desktopCfg = settings.desktop_app || {};
        const mode = desktopCfg.enabled ? "desktop" : "web";
        dom.uiForm.elements.ui_mode.value = mode;
        dom.uiForm.elements.suppress_browser.checked = desktopCfg.suppress_browser ?? true;
    }
    ...
    // Token direkt anwenden, falls er jetzt befuellt ist
    if (dom.tokenInput && dom.tokenInput.value.trim()) {
        state.lastAppliedToken = ""; // erzwinge erneutes Anwenden
        try {
            applyToken(true);
        } catch {
            // wird spaeter in bindEvents erneut aufgerufen
        }
    }
}
...
function bindEvents() {
    ...
    if (dom.uiForm) {
        dom.uiForm.addEventListener("submit", (event) => {
            event.preventDefault();
            const mode = dom.uiForm.elements.ui_mode.value || "web";
            const desktopEnabled = mode === "desktop";
            const payload = {
                desktop_app: {
                    enabled: desktopEnabled,
                    suppress_browser: Boolean(dom.uiForm.elements.suppress_browser.checked),
                },
            };
            apiFetch("/api/settings", {
                ...
                .then(() => {
                    showToast("UI-Modus gespeichert. Starte die App neu, um den Modus zu uebernehmen.", "success");
                    // Nur Form neu laden, nicht komplette Seite
                    return fetchJSON("/api/settings");
                })
                .then(({ settings }) => {
                    if (settings?.desktop_app) {
                        const cfg = settings.desktop_app;
                        dom.uiForm.elements.ui_mode.value = cfg.enabled ? "desktop" : "web";
                        dom.uiForm.elements.suppress_browser.checked = cfg.suppress_browser ?? true;
                    }
                })
                .catch((err) => {
                    console.error(err);
                    showToast("UI-Modus konnte nicht gespeichert werden.", "error");
                });
        });
    }
    if (dom.systemForm) {
    ...
    const savedToken = sessionStorage.getItem("jarvisWebToken");
    if (savedToken && savedToken.trim()) {
        if (dom.tokenInput.value.trim() !== savedToken.trim()) {
            dom.tokenInput.value = savedToken.trim();
        }
        applyToken(true);
    } else if (dom.tokenInput.value.trim()) {
        applyToken(true);
    }
    // Falls beim Start per Settings befuellt wurde, aber nicht in sessionStorage, anwenden
    if (!state.lastAppliedToken && dom.tokenInput.value.trim()) {
        applyToken(true);
    }
